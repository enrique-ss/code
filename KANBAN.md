...
Configurar projeto Phaser 3 (instalação, estrutura básica)
Criar estrutura de diretórios conforme Seção 3.16 (assets/events, assets/npcs, assets/maps, assets/classes, assets/data)
Configurar sistema de build/bundler (se necessário)
Configurar controle de versão Git
Criar arquivo README com instruções de setup
Criar arquivo `classes.json` com as 3 classes (Guerreiro, Mago, Arqueiro)
Criar arquivo `memorias.json` com definição de memórias do mundo
Criar arquivo `climas.json` com os 4 tipos de clima
Criar arquivo `eventos.json` com estrutura base (vazio inicialmente)
Criar arquivos de NPCs (mercador_vila.json, ferreiro.json, viajante.json)
Criar arquivos de mapas (campo.json, vila_inicial.json, estradas.json, floresta_sombria.json, ruinas_antigas.json)
